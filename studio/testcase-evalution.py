...
HUOM: API-avain luetaan .env-tiedostosta!
"""
...
class RAGASTestRunner:
    ...
    def __init__(
        self,
        test_file: str = "studio/testcases/Res_TestCases_20260130_093746.json",
        ...
        # Etsi projektin juuri (jossa on data-kansio tai studio-kansio)
        ...
        self.api_key = openai_api_key or os.getenv("EVAL_LLM_BINDING_API_KEY") or os.getenv("OPENAI_API_KEY") or os.getenv("LLM_BINDING_API_KEY")
        ...
        # User prompt: ensin parametri, sitten koodi, sitten ympäristömuuttuja
        if user_prompt is not None:
            self.user_prompt = user_prompt
        elif 'USER_PROMPT' in globals():
            self.user_prompt = USER_PROMPT
        else:
            self.user_prompt = os.getenv("LIGHTRAG_USER_PROMPT", "")
        
        # Tarkista API-avain
        if not self.api_key:
            print("❌ Virhe: API-avainta ei löydy!")
            print("Aseta LLM_BINDING_API_KEY .env-tiedostoon tai käytä OPENAI_API_KEY/EVAL_LLM_BINDING_API_KEY ympäristömuuttujaa")
            print("Esim .env-tiedostossa: LLM_BINDING_API_KEY=sk-your-key-here")
            sys.exit(1)
        
        # Tuloskansio projektin juureen
        self.results_dir = self.project_root / "studio" / "testresults"
        ...
        print("=" * 70)
        print(f"📂 Projektin juuri:      {self.project_root}")
        print(f"📁 Testitiedosto:        {self.test_file}")
        print(f"💾 Tuloshakemisto:       {self.results_dir}")
        ...
        print(f"📝 User Prompt:          {'✓ Asetettu' if self.user_prompt else '✗ Ei asetettu (käytetään oletusta)'}")
    ...
    def load_test_cases(self) -> List[Dict[str, Any]]:
        ...
        # Etsi tiedosto projektin juuresta
        search_paths = [
            self.project_root / self.test_file,
            Path(self.test_file),
            Path.cwd() / self.test_file,
        ]
    ...
    def _try_query_with_auth(self, question: str, query_mode: str, jwt_token: str = None, api_key: str = None, attempt_name: str = "Unknown") -> Dict[str, Any]:
        ...
        try:
            ...
            # Lisää user_prompt jos asetettu
            if self.user_prompt:
                query_payload["user_prompt"] = self.user_prompt
    ...
    def save_results(self, result: Dict[str, Any], dataset: Dataset, query_mode: str, duration: float, cost_info: Dict[str, Any]):
        ...
        # Tallenna JSON
        json_file = self.results_dir / f"tr_r_{query_mode}_{timestamp}.json"
        ...
        # Tallenna CSV
        try:
            csv_file = self.results_dir / f"tr_c_{query_mode}_{timestamp}.csv"
        ...
        md_file = self.results_dir / f"tr_{query_mode}_{timestamp}.md"
...
def main():
    ...
    parser.add_argument("--user-prompt", default=USER_PROMPT if 'USER_PROMPT' in globals() else "", 
                       help="Custom user prompt LightRAG:lle (tyhjä = käytä oletusta)")
