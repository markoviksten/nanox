...
# Etsi projektin juurihakemisto (jossa on data-kansio)
# Oletetaan että skripti voi olla missä tahansa projektin alihakemistossa
project_root = script_dir
while project_root.name and not (project_root / "data").exists():
    if project_root.parent == project_root:
        # Ollaan juuressa, ei löydy
        break
    project_root = project_root.parent

# Jos data-kansiota ei löydy, käytetään script_dir:iä
if not (project_root / "data").exists():
    project_root = script_dir

# Luo output-hakemisto projektin juureen
output_dir = project_root / "studio" / "testcases"
output_dir.mkdir(parents=True, exist_ok=True)

# Vaihda työhakemisto output-hakemistoon
os.chdir(output_dir)

# Luo output-tiedostonimi timestampilla (ilman polkua, koska ollaan jo oikeassa hakemistossa)
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
OUTPUT_FILE = Path(f"tc_{timestamp}.json")

print(f"Projektin juuri: {project_root}")
print(f"Skriptin sijainti: {script_dir}")
...
md_filename = Path(f"tcr_{timestamp}.md")
md_path = md_filename.absolute()
...
print(f"Generointi kesti: {duration:.1f}s")
print(f"JSON-tiedosto: {output_path}")
